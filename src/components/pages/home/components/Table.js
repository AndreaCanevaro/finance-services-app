import React from "react";
import "./Banner.css";
import HeaderBottom from "./HeaderBottom";

const Table = () => {
  return (
    <div style={{ overflowX: "auto" }} className="table-container">
      <table className="table">
        <thead>
          <tr>
            <th> </th>
            <th>Action</th>
            <th>Rank</th>
            <th>Strategy</th>
            <th>Quantity</th>
            <th>Entry Date</th>
            <th>Entry Price</th>
            <th>Current Price</th>
            <th>Current Value</th>
            <th>Return Percentage</th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              Pilbara Minerals (PLS:ASX)
            </td>
            <td data-label="Action">HOLD</td>
            <td data-label="Rank">1</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              South32 (S32:ASX)
            </td>
            <td data-label="Action">HOLD</td>
            <td data-label="Rank">2</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              IGO (IGO:ASX)
            </td>
            <td data-label="Action">HOLD</td>
            <td data-label="Rank">3</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              Woodside Petroleum (WPL:ASX)
            </td>
            <td className="buysell" data-label="Action">
              BUY
            </td>
            <td data-label="Rank">4</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              Computershare (CPU:ASX)
            </td>
            <td className="buysell" data-label="Action">
              BUY
            </td>
            <td data-label="Rank">5</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              Origin Energy (ORG:ASX)
            </td>
            <td className="buysell" data-label="Action">
              SELL
            </td>
            <td data-label="Rank">6</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>

        <tbody>
          <tr>
            <td className="table-names" data-label="Name">
              Metcash (MTS:ASX)
            </td>
            <td className="buysell" data-label="Action">
              SELL
            </td>
            <td data-label="Rank">7</td>
            <td data-label="Strategy">Mom</td>
            <td data-label="Quantity">1,188</td>
            <td data-label="Entry Date">15 Dec 21</td>
            <td data-label="Entry Price">$2.75</td>
            <td data-label="Current Price">$2.80</td>
            <td data-label="Current Value">$4,726.80</td>
            <td className="percentage" data-label="Return Percentage">
              16.37%
            </td>
          </tr>
        </tbody>
      </table>
      <HeaderBottom />
    </div>
  );
};

export default Table;
